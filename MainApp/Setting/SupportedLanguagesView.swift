    ...
    var body: some View {
        Form {
            Section("日本語") {
                ...
                LanguageFactRow("使い方") {
                    Text("初回の既定は日本語タブです。設定の「使用する言語」は編集可能な一覧で、日本語は先頭に固定され、英語は常に有効です。イタリア語を有効にすると英語との順番を並べ替えられます。QWERTYの地球儀キーは一覧の順に切り替わり、長押しで直接選べます。")
                }
            ...
            Section("英語") {
                ...
                LanguageFactRow("現在の制限") {
                    Text("任意の実験的な設定「ラテン文字の自動修正」は既定ではオフです。有効にすると、イタリア語・英語のよくあるタイプミスをスペース入力時に自動修正できます。数字・記号タブは選択中のラテン文字言語に合わせて表示されます。")
                }
            }
            ...
            Section {
                ...
                LanguageFactRow("使い方") {
                    Text("設定の「使用する言語」でイタリア語を有効にし、必要なら英語との順番を並べ替えます。日本語は先頭に固定され、英語は常に有効です。QWERTYの地球儀キーで有効な一覧を切り替え、長押しで「IT」を直接選べます。")
                }
                LanguageFactRow("現在の制限") {
