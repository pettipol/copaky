    ...
    var body: some View {
        Form {
            Section("日本語") {
                ...
                LanguageFactRow("使い方") {
                    Text("初回の既定は日本語タブです。設定の「使用する言語」は編集可能な一覧で、日本語は先頭に固定され、英語は常に有効です。イタリア語を有効にすると英語との順番を並べ替えられます。QWERTYの言語キーは一覧の順に切り替わり、長押しで直接選べます。")
                }
            ...
            Section {
                ...
                LanguageFactRow("使い方") {
                    Text("設定の「使用する言語」でイタリア語を有効にし、必要なら英語との順番を並べ替えます。日本語は先頭に固定され、英語は常に有効です。QWERTYの言語キーで有効な一覧を切り替え、長押しで「IT」を直接選べます。")
                }
                LanguageFactRow("現在の制限") {
